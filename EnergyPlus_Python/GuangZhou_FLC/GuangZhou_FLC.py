...
get_HR[index] = model.get('HR')  # 含湿量 Humidity Ratio
# 获取冷量（为了后面对2h进行平均）
get_Qs[index] = model.get('Qs')  # 获取显热冷量
get_Qt[index] = model.get('Qt')  # 获取全热冷量
get_Ql[index] = model.get('Ql')  # 获取显热冷量
get_Solar_diffuse[index] = model.get('Solar_diffuse')
get_Solar_direct[index] = model.get('Solar_direct')
# 获取室外温度（压缩机功率需要用到）
get_T_out[index] = model.get('T_out')  # 获取室外温度
to_T_supply[index] = model.get('T_supply')  # 获取系统节点温度 system node temperature（各个部件的节点温度）
...
plt.show()
plt.plot(t,cooling_load[0])
plt.plot(t,cooling_load[1])
plt.show()
# plt.plot(t,get)
